...
const Navigation = ({ unit, handleToggleUnit }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { data } = await axios(
        `https://api.openweathermap.org/data/2.5/weather?q=${value}&units=metric&appid=${apiKey}&units=metric`
      );
  ...
  const icon = 'https://www.iconfinder.com/icons/118810/web_browser_internet_icon'

  return (
    <>
      <NavWrapper>
        <Logo><Img src={icon}/>byForecast</Logo>
        ...
        <Cities cities={cities} handleRemove={handleRemove} unit={unit} />
      </NavWrapper>
      <Toggle unit={unit} handleToggleUnit={handleToggleUnit} />
    </>
  );
};
